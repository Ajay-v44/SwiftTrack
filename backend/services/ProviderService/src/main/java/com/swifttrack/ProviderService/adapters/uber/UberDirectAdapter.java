...
public class UberDirectAdapter implements DeliveryProvider {
                ...
                if (token == null || tokenExpireTime == null || tokenExpireTime < System.currentTimeMillis()) {
                        ...
                        try {
                                ...
                                if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                                        ...
                                        log.error("Failed to fetch Uber Direct token: HTTP {}",
                                ...
                                throw new RuntimeException(
                                                "Failed to authenticate with Uber Direct API: " + e.getMessage());
                        }
                }
        }
        ...
        public QuoteResponse getQuote(QuoteInput quoteInput) {
                ...
                try {
                        ...
                        if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                                ...
                                String currency = quoteResponse.getCurrencyType() != null
        ...
        public CreateOrderResponse createOrder(CreateOrderRequest createOrderRequest) {
                ...
                UberCreateOrderRequest uberRequest = UberCreateOrderRequest.builder()
                                ...
                                .externalStoreId(
                                                createOrderRequest.externalMetadata() != null
                                                                ? createOrderRequest.externalMetadata().storeId()
                                                                : "external-store-001")
                                .manifestTotalValue(
                                                createOrderRequest.packageInfo() != null
                                                                ? createOrderRequest.packageInfo().totalValue()
                                                                : 0)
                                ...
                                .testSpecifications(UberCreateOrderRequest.TestSpecifications.builder()
                                                .roboCourierSpecification(
                                                                UberCreateOrderRequest.RoboCourierSpecification
                                                                                .builder().mode("auto").build())
                                                .build())
                                .build();
                ...
                if (createOrderRequest.items() != null && !createOrderRequest.items().isEmpty()) {
                        uberRequest.setManifestItems(createOrderRequest.items().stream()
                                        .map(item -> UberCreateOrderRequest.ManifestItem.builder()
                                                        ...
                                                        .dimensions(item.dimensionsCm() != null
                                                                        ? UberCreateOrderRequest.Dimensions.builder()
                                                                                        .length(item.dimensionsCm()
                                                                                                        .length())
                                                                                        .height(item.dimensionsCm()
                                                                                                        .height())
                                                                                        .depth(item.dimensionsCm()
                                                                                                        .width()) // using
                                                                                                                  // width
                                                                                                                  // for
                                                                                                                  // depth
        ...
        public Message cancelOrder(String orderId) {
                try {
                        ...
                        if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                                ...
                                log.error("Failed to cancel Uber Direct order: HTTP {}", response.getStatusCode());
                                throw new RuntimeException(
                                                "Failed to cancel Uber Direct order: HTTP " + response.getStatusCode());
                        }
                } catch (Exception e) {
                        log.error("Exception while canceling Uber Direct order: {}", e.getMessage(), e);
                        throw new RuntimeException("Failed to cancel Uber Direct order: " + e.getMessage());
                }
        }
}
